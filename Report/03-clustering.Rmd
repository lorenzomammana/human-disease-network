...
Il primo algoritmo di clustering che abbiamo utilizzato è basato sull'utilizzo della misura di edge betweennes. \newline
...
plot_pretty_graph(UD_nogenes, "graphopt", betweennes.communities$membership, length(betweennes.communities), E(UD_nogenes)$weight, "Girvan–Newman Clusters")
```
...
plot_pretty_graph(UD_nogenes, "graphopt", fgreedy.communities$membership, length(fgreedy.communities), E(UD_nogenes)$weight, "Fastgreedy Clusters")
```
...
plot_pretty_graph(UD_nogenes, "graphopt", louvain.communities$membership, length(louvain.communities), E(UD_nogenes)$weight, "Louvain Clusters")
```
...
plot_pretty_graph(UD_nogenes, "graphopt", spinglass.communities$membership, length(spinglass.communities), E(UD_nogenes)$weight, "Spinglass Clusters")
```
...
plot_pretty_graph(UD_nogenes, "graphopt", mcl.communities$Cluster, length(mcl.communities), 
...
plot_pretty_graph(UD_nogenes, "graphopt", leiden.communities$membership, length(leiden.communities), E(UD_nogenes)$weight, "Leiden Clusters")
```
...
plot_pretty_graph(UD_nogenes, "graphopt", label_prop.communities$membership, length(label_prop.communities), E(UD_nogenes)$weight, "Label Propagation Clusters")
```
...
plot_pretty_graph(UD_nogenes, "graphopt", label_prop_init.communities$membership, length(label_prop_init.communities), E(UD_nogenes)$weight, "Label Propagation init Clusters")
```
...
rownames(accuracy_df) <- c("betweennes", "fastgreedy", "louvain", "spinglass", "markov",
                           "leiden", "label prop", "init label prop")
...
panderOptions('table.split.table', 8*12)
