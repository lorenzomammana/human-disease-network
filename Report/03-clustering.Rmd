...
{
  fgreedy_cluster_df$name[c] <- max(names(table(V(UD_nogenes)[fgreedy.communities[[c]]]$X1)))
}
...
{
  louvain_cluster_df$name[c] <- max(names(table(V(UD_nogenes)[UD_louvain[[c]]]$X1)))
}
...
for (c in 1:length(UD_springlass$csize))
{
  spinglass_cluster_df$name[c] <- max(names(table(V(UD_nogenes)[UD_spinglass[[c]]]$X1)))
}
...
{
  markov_cluster_df$name[c+1] <- max(names(table(V(UD_nogenes)[mcl_clusters$Cluster == c]$X1)))
}
