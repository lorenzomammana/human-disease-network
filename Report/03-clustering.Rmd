...
```{r componente1, echo=FALSE, fig.height=8, fig.width=8}
grid.arrange(grobs = plots[1:4])
```
```{r componente2, echo=FALSE, fig.height=8, fig.width=8}
grid.arrange(grobs = plots[5:8])
```
```{r componente3, echo=FALSE, fig.height=8, fig.width=8}
grid.arrange(grobs = plots[9:12])
```
```{r componente4, echo=FALSE, fig.height=8, fig.width=8}
grid.arrange(grobs = plots[13:16])
```
```{r componente5, echo=FALSE, fig.height=8, fig.width=8}
grid.arrange(grobs = plots[17:20])
```
```{r componente6, echo=FALSE, fig.height=8, fig.width=8}
...
label_cluster <- function(clusters)
{
  ...
  for (c in 1:length(clusters))
  {
    ...
    {
      cluster_df$name[cluster_df$id == c] <- V(UD_nogenes)[clusters[[c]]]$X1
    }
  }
...
dovrebbero avere un alto valore di edge betweenness. 
...
```{r include=FALSE}
betweennes_cluster_df <- label_cluster(betweenness.communities)
...
DA MODIFICARE: Abbiamo ripetuto bla, bla bla
...
accuracy_df <- data.frame(matrix(ncol = 23, nrow = 9))
colnames(accuracy_df) <- c(disease_clusters, "Purity")
rownames(accuracy_df) <- c("betweenness", "fastgreedy", "louvain", "spinglass", "markov",
...
accuracy_df$Unclassified <- NULL

accuracy_df <- as.data.frame(t(accuracy_df))
colnames(accuracy_df) <- accuracy_df[1, ]

rownames(accuracy_df) <- c("Ear,Nose...", "Cancer", "Opthalmolog", "Endocrine",
...
temp <- accuracy_df[14, ]
accuracy_df[14, ] <- accuracy_df[20, ]
accuracy_df[20, ] <- temp

colnames(accuracy_df) <- c("betweenness", "fastgreedy", "louvain", "spinglass", "markov",
                          "leiden", "label prop", "label prop init", "lead eigenvector")
```

```{r}
panderOptions('table.split.table', 8*10)
...
La Normalized Mutual Information rispetto alla purity è una misura più penalizzante, è calcolata con la seguetne formula:
...
\newpage
```{r}
panderOptions('table.split.table', 8*14)
...
- **Aquaporin-1 deficiency**: viene identificata da tutti e 7 gli algoritmi come appartenente alla community *Hematological*. Verificando meglio attraverso la fonte \textcolor{blue}{[OMIM - Online Mendelian Inheritance in Man](https://omim.org)}, una trattazione sintetica di geni umani e fenotipi genetici, è possibile verificare la relazione di questo disturbo con i gruppi sanguigni.  
- **Benzene toxicity**: viene indentificata da tutti gli algoritmi come appartenente al cluster *Cancer*. In questo caso questo tipo di malattia sembreremme ricevere numerose influenze da Leukemia, Post-chemotherapy, Breast cancer etc.  
- **Bannayan-Riley- Ruvalcaba syndrome**: viene identificata da tutti gli algoritmi (meno uno FIXME) come appartenente alla community *Cancer*. Collegamento con il cancro al seno.
...
In questo caso, risulta evidente come in alcuni casi le malattie non si inseriscano più in diversi cluster, ma alcune di queste presentano un unica community ricorrente. FIXME  
...
In altri casi invece, come per **Waardenburg syndrome**, **Stickler syndrome** e **Waardenburg-Shah syndrome** multipli cluster sembrano influire sulle malattie.
