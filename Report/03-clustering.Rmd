...
```{r}
fgreedy.communities <- fastgreedy.community(UD_nogenes, modularity = TRUE, weights = E(UD_nogenes)$weight) #26 communities
...
```{r}
UD_springlass <- cluster_spinglass(UD_nogenes, spins = 225, weights = E(UD_nogenes)$weight)
...
ggraph(UD_nogenes, layout = 'kk') +
  geom_edge_fan(colour = "gray") + 
  geom_node_point() +
  geom_node_point(aes(colour = mcl_clusters$Cluster)) + ## FIXME Error: Aesthetics must be either length 1 or the same as the data (1419): colour
  labs(col="cluster") +
